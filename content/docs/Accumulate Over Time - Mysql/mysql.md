...
- 如何将列的数据变为行的数据呢？通过可以采用 `group by` + 特定的mysql语句来完成
...
>***Tip***
>
>这道题本质也是行专列的模式类型，不同的是如何将使得每个学生按照姓名的字母顺序依次排列在对应的大洲下面。（对的就是进行排序，如果我们对以及分组的进行排序时不可以的🙅‍）
>
>因此，我们需要在分组的过程中就对其进行排序。我们需要使用到窗口函数，它可以帮组我们在分组中对透视表进行排序。
>
>获得排序后的结果
>
>```mysql
...
>```
>
>分组排序之后，在对结果进行行转列操作。题目说明只有三个大洲，因此不要我们考虑太复杂的操作。
>
>```mysql
>
>select 
...
>```
>
>摘自：[618.(Hard)学生地理信息报告](https://zqt0.gitbook.io/leetcode/sql/618.students-report-by-geography)
